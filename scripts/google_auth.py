#!/usr/bin/env python3
"""
Google API credential management for Hermes SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/hermes-seo/google-api.json")
TOKEN_PATH = os.path.expanduser("~/.config/hermes-seo/oauth-token.json")
...
def load_config() -> dict:
    ...
    Reads ~/.config/hermes-seo/google-api.json first. Any missing fields
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for Hermes SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
