#!/usr/bin/env python3
"""
Backlink API credential management for Hermes SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/hermes-seo/backlinks-api.json")
CACHE_DIR = os.path.expanduser("~/.cache/hermes-seo/commoncrawl")
...
def load_config() -> dict:
    ...
    Reads ~/.config/hermes-seo/backlinks-api.json first. Any missing fields
...
Free backlink data sources for Hermes SEO. No payment required for any of these.
...
def main():
    parser = argparse.ArgumentParser(
        description="Backlink API credential management for Hermes SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
