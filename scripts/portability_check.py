...
hermes-seo skills run under Hermes Agent, but also Cursor / Cursor Cloud
...
def check_one(path: Path) -> list[dict]:
    ...
    # strictly than Hermes Agent — warn if there are inline comments
        ...
        if 'version' not in frontmatter["metadata"]:
            findings.append({
                ...
                "message": "metadata block present but no version field; "
                           "hermes-seo's manifest test expects "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Cross-platform portability lint for hermes-seo SKILL.md files."
    )
