...
1. Open a private [GitHub Security Advisory](https://github.com/somarjun/hermes-seo/security/advisories/new) on this repository (preferred channel).
...
hermes-seo follows a **90-day coordinated disclosure** policy.
...
hermes-seo is a research and audit toolkit that runs on a user's workstation. It accepts user-supplied URLs and credentials, and issues HTTP requests against arbitrary internet hosts. The threat model has three primary attacker types:

1. **Malicious audit target.** A site the user points hermes-seo at attempts to leak local-network or cloud-metadata data via SSRF chains: private IP literals, decimal/hex/octal IPv4, FQDN trailing dot, 30x redirects to private IPs, DNS rebinding (initial public resolution → later private), IPv4-mapped IPv6, dual-stack hosts with one private record.
...
3. **Local privilege escalation against stored credentials.** The OAuth token at `~/.config/hermes-seo/oauth-token.json` is the most sensitive on-disk artifact.
...
- **Playwright + Chromium DNS rebinding.** Chromium does its own DNS resolution inside the renderer process. hermes-seo's Python-layer DNS pin (`url_safety._pin_dns`) cannot reach it. The Playwright `route()` handler re-validates every subresource host (`make_safe_playwright_route_handler`), which closes the common case, but a true rebinding attacker can still race Chromium's resolver after our pre-flight returns. Mitigation: do not point `/seo` skills at untrusted sites with high-frequency redirects.
...
- Install scripts write only to user-level directories under `~/.claude/` and `~/.config/hermes-seo/`.
- Python dependencies install into an isolated virtual environment at `~/.hermes/skills/seo/.venv/`.
