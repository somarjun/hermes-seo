...
- This release rolls forward two commits that landed on main after the v1.9.7
  ...
  - `8514999`: marketplace metadata polish (added `category: "marketing"`,
    `homepage: https://hermes-seo.md`, and a 14-keyword array to the
...
### License
- FLOW content bundled under CC BY 4.0. Attribution header on every prompt file (automated by `sync_flow.py`). Hermes SEO's MIT license unchanged, applies to skill code only.
...
- **Hardcoded absolute paths**: removed `~/.hermes/skills/` from `agents/seo-visual.md`, `agents/seo-schema.md`, `skills/seo-image-gen/SKILL.md`, and banana extension copy (now use plugin-relative paths)
...
### Added
- **Marketplace distribution**: Created `.claude-plugin/marketplace.json` for plugin marketplace submission. Users can now install via `/plugin marketplace add somarjun/hermes-seo`
...
### Security
- **Install script supply chain fix**: Replaced `irm | iex` Windows PowerShell one-liner with `git clone + powershell -File` as primary install method. Hermes Agent's own security guardrails flagged the old pattern as a supply chain risk (reported by community member). Added collapsible "review before running" section for Unix curl method.
- **Version pinning**: `install.sh` and `install.ps1` now clone a specific release tag (`v1.3.0`) by default rather than `main`, preventing silent updates. Override with `HERMES_SEO_TAG=main`.
...
### Added
- **install.cat support**: Added alternative install method via `curl install.cat/somarjun/hermes-seo | bash` to README (#10)
...
- **requirements.txt**: Now persisted to `~/.hermes/skills/seo/` for user retry
...
### Changed
- Python dependencies now installed in a venv at `~/.hermes/skills/seo/.venv/` with `--user` fallback (#2)
...
### Architecture
- Follows Anthropic's official Hermes Agent skill specification (February 2026)
