# Hermes SEO: Multi-Platform Agent Instructions
...
>
> Hermes Agent users: see `CLAUDE.md` instead.

## Cross-platform portability (v2.0.0)

Every skill in `skills/*/SKILL.md` is authored to a portable subset of the
Hermes Agent skill spec. Validate compatibility with your harness via:
...
| Harness | How to load hermes-seo |
|---|---|
...
Where hermes-seo skills mention Hermes Agent tools (`Read`, `Write`, `Edit`,
`Bash`, `Glob`, `Grep`, `WebFetch`), each harness typically has an equivalent:

| Hermes Agent | Codex | Cline | Aider | Cursor / Antigravity |
|---|---|---|---|---|
...
Hermes SEO is a Tier 4 SEO analysis skill with 25 sub-skills (21 core + 1 orchestrator +
...
- PATH may not include Python venv. Use full path: `~/.hermes/skills/seo/.venv/bin/python`
...
Place the repo in `~/.gemini/antigravity/plugins/hermes-seo/` or install via:
...
4. **Config location**: `~/.config/hermes-seo/` for API credentials
