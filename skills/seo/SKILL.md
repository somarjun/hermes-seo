...
When the user invokes `/seo audit`, delegate via `delegate_task` in parallel:
1. Detect business type (SaaS, local, ecommerce, publisher, agency, other)
2. Use `delegate_task(tasks=[...])` in parallel (max 3 concurrent by default; raise `delegation.max_concurrent_children` in config for full audits). Load delegate briefs from `references/delegates/`: seo-technical, seo-content, seo-schema, seo-sitemap, seo-performance, seo-visual, seo-geo
3. If Google API credentials detected (`python ${HERMES_SKILL_DIR}/scripts/google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`python ${HERMES_SKILL_DIR}/scripts/backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`python ${HERMES_SKILL_DIR}/scripts/drift_history.py <url>`), also spawn seo-drift agent
...
strategy. hermes-seo uses a 10-principle thinking framework grouped into four
...
## Subagents (Hermes: `delegate_task`)

For parallel analysis during audits, use `delegate_task(tasks=[...])`. Each task's
`goal` should include the delegate brief from `references/delegates/<name>.md`.
Pass the target URL and any crawl context in `context`. Use
...
```python
delegate_task(tasks=[
    {"goal": "Run seo-technical audit per references/delegates/seo-technical.md",
     "context": "URL: https://example.com", "toolsets": ["terminal", "file", "web"]},
    {"goal": "Run seo-content audit per references/delegates/seo-content.md",
     "context": "URL: https://example.com", "toolsets": ["terminal", "file", "web"]},
])
```

Or invoke the bundled workflow: `/seo-audit https://example.com`

Delegate briefs available:
