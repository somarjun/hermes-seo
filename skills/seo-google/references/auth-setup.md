...
3. Name it (e.g., "Hermes SEO") and note the project ID
...
2. Name: `hermes-seo` (or similar)
3. Skip optional permissions steps
4. Click on the created service account > **Keys > Add Key > Create new key > JSON**
5. Download the JSON file and store it securely (e.g., `~/.config/hermes-seo/service_account.json`)
...
```json
{
  ...
  "client_email": "hermes-seo@your-project.iam.gserviceaccount.com",
  "client_id": "...",
...
```bash
mkdir -p ~/.config/hermes-seo
```

Save to `~/.config/hermes-seo/google-api.json`:

```json
{
  "service_account_path": "~/.config/hermes-seo/service_account.json",
  "api_key": "AIzaSy...",
...
Credential Tier: 2 -- Full (API key + Service Account + GA4)
  ...
  [OK] Google Search Console API
       Service account: hermes-seo@your-project.iam.gserviceaccount.com
