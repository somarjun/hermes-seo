...
instead of WebSearch for SERP data. Run `python ${HERMES_SKILL_DIR}/scripts/dataforseo_costs.py check serp_organic_live_advanced --count N`
...
```
Test: Does ~/.hermes/skills/blog/SKILL.md exist?
```
...
- All URLs fetched via `python ${HERMES_SKILL_DIR}/scripts/fetch_page.py` (SSRF protection via `validate_url()`)
