...
```
python ${HERMES_SKILL_DIR}/scripts/dataforseo_costs.py check <endpoint> [--count N]
```
...
```
python ${HERMES_SKILL_DIR}/scripts/dataforseo_costs.py log <endpoint> <actual_cost>
```
...
When DataForSEO MCP tools are available, other hermes-seo skills can leverage live data:
...
Match existing hermes-seo output patterns:
