...
```bash
head -5 ~/.hermes/skills/seo/SKILL.md
```
...
```bash
ls ~/.hermes/skills/seo/requirements.txt
```

If missing, download it directly:
```bash
curl -fsSL https://raw.githubusercontent.com/somarjun/hermes-seo/main/requirements.txt \
  -o ~/.hermes/skills/seo/requirements.txt
```
...
```bash
ls ~/.hermes/skills/seo/references/delegates/seo-*.md
```

2. Check agent frontmatter:
```bash
head -5 ~/.hermes/skills/seo/references/delegates/seo-technical.md
```

3. Re-install agents:
```bash
cp /path/to/hermes-seo/agents/*.md ~/.hermes/skills/seo/references/delegates/
```
...
3. **Logs:** Check Hermes Agent's output for error details

## Debug Mode

To see detailed output, check Hermes Agent's internal logs or run scripts directly:

```bash
# Test fetch
python3 ~/.hermes/skills/seo/scripts/fetch_page.py https://example.com

# Test parse
python3 ~/.hermes/skills/seo/scripts/parse_html.py page.html --json

# Test screenshot
python3 ~/.hermes/skills/seo/scripts/capture_screenshot.py https://example.com
```
