...
If the installer fails, add this to `~/.hermes/config.yaml` manually:
...
Start Hermes Agent and try:

```
/seo firecrawl map https://example.com
```

You should see a list of discovered URLs. If you get a "tool not available" error, restart Hermes Agent to reload MCP servers.
