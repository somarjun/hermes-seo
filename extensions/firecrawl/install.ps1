# Firecrawl Extension Installer for Hermes SEO (Windows)
...
if (-not (Test-Path $SeoSkillDir)) {
    Write-Host "x Hermes SEO is not installed." -ForegroundColor Red
    Write-Host "  Install it first: irm https://raw.githubusercontent.com/somarjun/hermes-seo/main/install.ps1 | iex"
    exit 1
}
Write-Host "v Hermes SEO detected" -ForegroundColor Green
