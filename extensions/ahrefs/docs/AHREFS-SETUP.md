...
server into your Hermes Agent session so the `seo-ahrefs` skill can call
live Ahrefs data.
...
4. Copies `skills/seo-ahrefs/SKILL.md` into `~/.hermes/skills/seo-ahrefs/`.
5. Atomically writes `mcpServers.ahrefs` into `~/.hermes/config.yaml`
   with your token in the `env` block. The settings file is `chmod 0o600`
   after the merge (same hardening as the OAuth token).

## Verify

Open a new Hermes Agent session and ask:
...
`scripts/dataforseo_costs.py` cost tracker shipped with hermes-seo
