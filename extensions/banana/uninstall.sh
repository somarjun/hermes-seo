...
main() {
    ...
    rm -rf "${HOME}/.hermes/skills/seo-image-gen"

    # Remove agent
    rm -f "${HOME}/.hermes/skills/seo/references/delegates/seo-image-gen.md"
    ...
    if [ -f "${SETTINGS_FILE}" ]; then
        ...
        if [ -d "${HOME}/.hermes/skills/banana" ]; then
            echo "  ℹ  Standalone banana skill detected at ~/.hermes/skills/banana/"
...
" 2>/dev/null || echo "  ⚠  Could not auto-remove MCP config. Remove 'nanobanana-mcp' from ~/.hermes/config.yaml manually."
        fi
    fi
