...
```bash
python3 ~/.hermes/skills/seo-image-gen/scripts/validate_setup.py
```

Or check manually:
1. `ls ~/.hermes/skills/seo-image-gen/SKILL.md`:skill file exists
2. `ls ~/.hermes/skills/seo/references/delegates/seo-image-gen.md`:agent file exists
3. `grep nanobanana ~/.hermes/config.yaml`:MCP configured

## Common Issues

### "MCP tools not available"
- Restart Hermes Agent after installing the extension
- Verify your API key is valid at [aistudio.google.com](https://aistudio.google.com)
- Check `~/.hermes/config.yaml` has the nanobanana-mcp entry
