...
# DataForSEO Extension Installer for Hermes SEO
# Wraps everything in main() to prevent partial execution on network failure

main() {
    SKILL_DIR="${HOME}/.hermes/skills/seo-dataforseo"
    AGENT_DIR="${HOME}/.hermes/skills/seo/references/delegates"
    SEO_SKILL_DIR="${HOME}/.hermes/skills/seo"
    ...
    # Support both traditional (curl|bash → ~/.hermes/skills/seo) and marketplace
    ...
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        _GLOB_MATCH=$(ls -d "${HOME}/.claude/plugins/cache/agricidaniel-seo/hermes-seo/"*/skills/seo 2>/dev/null | tail -n1 || true)
        [ -n "${_GLOB_MATCH}" ] && [ -d "${_GLOB_MATCH}" ] && SEO_SKILL_DIR="${_GLOB_MATCH}"
    fi

    # Check prerequisites
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        echo "✗ Hermes SEO is not installed."
        echo "  Install it first: curl -fsSL https://raw.githubusercontent.com/somarjun/hermes-seo/main/install.sh | bash"
        exit 1
    fi
    echo "✓ Hermes SEO detected"
    ...
    if [ -f "${SCRIPT_DIR}/skills/seo-dataforseo/SKILL.md" ]; then
        ...
        echo "  Run this script from the hermes-seo repo: ./extensions/dataforseo/install.sh"
        exit 1
    fi
...
" || {
        ...
        echo "  Add the dataforseo server manually to ~/.hermes/config.yaml"
    ...
    echo ""
    echo "Usage:"
    echo "  1. Start Hermes Agent:  claude"
    echo "  2. Run commands:"
