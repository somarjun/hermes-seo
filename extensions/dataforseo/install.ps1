# DataForSEO Extension Installer for Hermes SEO (Windows)
...
Write-Host "║   For Hermes SEO                     ║" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $SeoSkillDir)) {
    Write-Host "✗ Hermes SEO is not installed." -ForegroundColor Red
    Write-Host "  Install it first: irm https://raw.githubusercontent.com/somarjun/hermes-seo/main/install.ps1 | iex"
    exit 1
}
Write-Host "✓ Hermes SEO detected" -ForegroundColor Green
...
if (Test-Path "$ScriptDir\skills\seo-dataforseo\SKILL.md") {
    ...
    Write-Host "  Run this script from the hermes-seo repo."
    exit 1
}
...
Write-Host "  1. Start Hermes Agent:  claude"
